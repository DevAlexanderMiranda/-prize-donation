...
// Elementos DOM
const topPremiosTableBody = document
  .getElementById("top-premios-table")
  .querySelector("tbody");
const listaColaboradoresTableBody = document
  .getElementById("lista-colaboradores-table")
  .querySelector("tbody");
let dadosColaboradores = []; // Armazenar dados originais de colaboradores para filtro
...
async function checkUserPermissions() {
  ...
  try {
    ...
    if (querySnapshot.empty) {
      console.error(
        "Usuário não encontrado pelo UID no Firestore:",
        user.email
      );
      ...
      if (!emailQuerySnapshot.empty) {
        console.log(
          "Usuário encontrado pelo email! Atualizando UID no documento..."
        );
...
async function carregarDadosDashboard() {
  try {
    ...
    doacoesSnapshot.forEach((doc) => {
      ...
      const colaboradorKey = doacao.colaborador || "Não especificado";
      if (!colaboradores[colaboradorKey]) {
        colaboradores[colaboradorKey] = [];
      }
      colaboradores[colaboradorKey].push({
        premioNome: premioKey,
    ...
    // Preencher a tabela de colaboradores
    preencherTabelaColaboradores(colaboradores);
  } catch (error) {
...
function preencherTabelaTopPremios(premios) {
  ...
  if (topPremios.length === 0) {
    const row = document.createElement("tr");
    row.innerHTML =
      '<td colspan="3" style="text-align: center;">Nenhum prêmio cadastrado</td>';
    topPremiosTableBody.appendChild(row);
  }
}
...
function preencherTabelaListaPremios(premios, totalPremios) {
  ...
  // Referência à tabela
  const listaPremiosTableBody = document
    .getElementById("lista-premios-table")
    .querySelector("tbody");
  ...
  premiosArray.forEach((premio) => {
    ...
    const percentual =
      totalPremios > 0
        ? ((premio.quantidade / totalPremios) * 100).toFixed(1)
        : 0;
  ...
  if (premiosArray.length === 0) {
    const row = document.createElement("tr");
    row.innerHTML =
      '<td colspan="4" style="text-align: center;">Nenhum prêmio cadastrado</td>';
    listaPremiosTableBody.appendChild(row);
  }
}
...
function preencherTabelaListaSecretarias(secretarias, totalPremios) {
  ...
  // Referência à tabela
  const listaSecretariasTableBody = document
    .getElementById("lista-secretarias-table")
    .querySelector("tbody");
  ...
  secretariasArray.forEach((secretaria) => {
    ...
    const percentual =
      totalPremios > 0
        ? ((secretaria.totalPremios / totalPremios) * 100).toFixed(1)
        : 0;
  ...
  if (secretariasArray.length === 0) {
    const row = document.createElement("tr");
    row.innerHTML =
      '<td colspan="4" style="text-align: center;">Nenhuma secretaria/empresa cadastrada</td>';
    listaSecretariasTableBody.appendChild(row);
  }
}

// Função para preencher a tabela de colaboradores
function preencherTabelaColaboradores(colaboradores, filtro = "") {
  if (!listaColaboradoresTableBody) {
    console.error("Elemento da tabela de colaboradores não encontrado");
    return;
  }

  // Limpar tabela
  listaColaboradoresTableBody.innerHTML = "";

  // Transformar objeto em array para ordenação
  const colaboradoresArray = Object.keys(colaboradores).map((nome) => {
    return {
      nome: nome,
      doacoes: colaboradores[nome],
    };
  });

  // Ordenar por nome do colaborador
  colaboradoresArray.sort((a, b) => a.nome.localeCompare(b.nome));

  // Armazenar dados originais para uso no filtro
  if (filtro === "") {
    dadosColaboradores = [];
  }
  ...
  colaboradoresArray.forEach((colaborador) => {
    colaborador.doacoes.forEach((doacao) => {
      // Se houver filtro, verificar se o nome do colaborador ou prêmio corresponde
      const filtroBaixo = filtro.toLowerCase();
      const colaboradorBaixo = colaborador.nome.toLowerCase();
      const premioBaixo = doacao.premioNome.toLowerCase();

      if (
        filtro === "" ||
        colaboradorBaixo.includes(filtroBaixo) ||
        premioBaixo.includes(filtroBaixo)
      ) {
        const row = document.createElement("tr");

        const nomeCell = document.createElement("td");
        nomeCell.textContent = colaborador.nome;

        const premioCell = document.createElement("td");
        premioCell.textContent = doacao.premioNome;

        const quantidadeCell = document.createElement("td");
        quantidadeCell.textContent = doacao.quantidade;

        row.appendChild(nomeCell);
        row.appendChild(premioCell);
        row.appendChild(quantidadeCell);

        listaColaboradoresTableBody.appendChild(row);

        // Armazenar dados para uso no filtro
        if (filtro === "") {
          dadosColaboradores.push({
            colaborador: colaborador.nome,
            premio: doacao.premioNome,
...
onAuthStateChanged(auth, async (user) => {
  console.log(
    "Estado da autenticação alterado:",
    user ? `Usuário ${user.email} autenticado` : "Usuário não autenticado"
  );
  ...
  if (user) {
    try {
      ...
      if (querySnapshot.empty) {
        console.error(
          "Usuário não encontrado após verificação de permissões, isso é inesperado"
        );
        window.location.href = "../index.html";
        return;
      }
...
// Event listener para o campo de pesquisa de colaboradores
document.addEventListener("DOMContentLoaded", function () {
  const pesquisaColaborador = document.getElementById("pesquisa-colaborador");
  const btnPesquisarColaborador = document.getElementById(
    "btn-pesquisar-colaborador"
  );
  const btnLimparPesquisaColaborador = document.getElementById(
    "btn-limpar-pesquisa-colaborador"
  );

  if (
    pesquisaColaborador &&
    btnPesquisarColaborador &&
    btnLimparPesquisaColaborador
  ) {
    // Pesquisar ao clicar no botão
    btnPesquisarColaborador.addEventListener("click", function () {
      filtrarColaboradores();
    });

    // Pesquisar ao pressionar Enter
    pesquisaColaborador.addEventListener("keyup", function (event) {
      if (event.key === "Enter") {
        filtrarColaboradores();
      }
    });
    ...
    // Limpar pesquisa
    btnLimparPesquisaColaborador.addEventListener("click", function () {
      pesquisaColaborador.value = "";
      filtrarColaboradores();
    });
  }
});
...
function filtrarColaboradores() {
  const pesquisaColaborador = document.getElementById("pesquisa-colaborador");
  if (!pesquisaColaborador) return;

  const termoPesquisa = pesquisaColaborador.value.trim();

  // Reconstruir objeto colaboradores a partir dos dados armazenados
  const colaboradores = {};

  dadosColaboradores.forEach((item) => {
    if (!colaboradores[item.colaborador]) {
      colaboradores[item.colaborador] = [];
    }

    colaboradores[item.colaborador].push({
      premioNome: item.premio,
      quantidade: item.quantidade,
    });
  });

  // Atualizar tabela com o filtro
  preencherTabelaColaboradores(colaboradores, termoPesquisa);
}
