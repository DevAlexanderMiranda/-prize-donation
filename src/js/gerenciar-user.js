...
async function loadUsers() {
  try {
    ...
    usersTableBody.innerHTML =
      '<tr><td colspan="5" style="text-align: center;">Erro ao carregar usuários</td></tr>';
  }
}
  ...
  try {
    if (editingUserId) {
      ...
      if (!password) {
        showToast(
          "Erro",
          "Por favor, forneça uma senha para o novo usuário.",
          "error"
        );
        return;
      }
        ...
        if (authError.code === "auth/email-already-in-use") {
          ...
          try {
            ...
            if (querySnapshot.empty) {
              showToast(
                "Erro",
                "Este email já está em uso no sistema de autenticação, mas não está registrado no banco de dados. Por favor, contate o administrador para resolver este problema.",
                "error"
              );
            } else {
              showToast(
                "Erro",
                "Este email já está cadastrado no sistema. Por favor, use outro email.",
                "error"
              );
            }
          } catch (error) {
          ...
          showToast(
            "Erro",
            "A criação de usuários com email/senha não está habilitada. Por favor, contate o administrador.",
            "error"
          );
        } else {
          showToast(
